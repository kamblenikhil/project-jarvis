...
// Encrypts (device_id, command_id) with AES-CTR using counter as nonce,
// computes HMAC-SHA256(magic||counter||ct)[:8].
bool jarvis_seal(uint32_t counter, uint8_t device_id, uint8_t command_id,
                 JarvisWire *out);

// Verifies MAC, decrypts ciphertext. Returns true if MAC matches.
